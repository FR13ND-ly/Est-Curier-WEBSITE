...
def getFastInfo(request):
    return JsonResponse(Article.objects.filter(draft=True).count(), safe=False)
    ...
    for urlRaw in article.moreArticles.split(','):
        ...
        if (url.replace('[', '').replace(']', '')):
            ...
            if (moreArticle.count()):
                moreArticle = moreArticle[0]
                moreArticles.append({
                    "id": moreArticle.id,
                    "title": moreArticle.title,
                    "cover":apiUrl + File.objects.get(id=moreArticle.cover).file.url if isinstance(article.cover, str) else '',
                    "url": moreArticle.url
                })
    response = {
        "id": article.id,
...
def getSurvey(request):
    data = JSONParser().parse(request)
    surveysRaw = Survey.objects.filter(article=data['id'])
    surveys = []
...
def updateYTvideos(request):
    for YTvideo in YoutubeVideo.objects.all():
        YTvideo.delete()
...
def addMoreArticles(moreArticles, id):
    ...
    articles = Article.objects.filter(
        tags__contains=tag).filter(draft=False).order_by("-date")
    for article in articles:
...
def uploadPdf(request):
    ...
    for page in doc.pages():
        pix = page.getPixmap()
        pix.writePNG(settings.MEDIA_ROOT + "/" +
                     file.name + str(i + 1) + ".png")
